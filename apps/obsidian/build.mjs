...
function runGates() {
	...
	try {
		...
		execFileSync("node", ["tools/check-classes.mjs", "registry", ...alsoInVault], { stdio: "inherit" });
		execFileSync("node", ["tools/check-adaptive.mjs", "registry", ...alsoInVault], { stdio: "inherit" });
		execFileSync("node", ["tools/check-overrides.mjs", "apps/obsidian/styles.css"], { stdio: "inherit" });
	} catch {
		process.exit(1);
	}
}

...
function lintNode(lint, node, path, parent) {
	...
	HEIGHT_WAS.filter((key) => node[key] !== undefined).forEach((key) =>
		said(`"${key}" is gone: a widget is as tall as what it draws, and only a region's width is set by hand, so remove it`),
	);

...
import { diceBearUrl, diceBearVerdict } from "./dicebear-styles.js";
...
const GLYPHS = {
	...
	tick: '<path d="M4.6 10.4l3.7 3.7 7.1-7.9"/>',
...
const EMBLEM_SIZES = { s: 24, m: 40, l: 64, xl: 96 };
const EMBLEM_SHAPES = ["circle", "rounded", "square"];
const EMBLEM_STATUS = createContext(null);

export function Emblem({ size = "m", shape = "circle", label, className: cls, style, children }) {
...
function EmblemImage({ src, alt = "", className: cls, ...rest }) {
	...
	return h("img", { ...rest, src, alt, className: cx("wg-kit-emblem-image", cls), draggable: false });
}

function EmblemDiceBear({ style, seed, options, className: cls }) {
	...
	if (verdict.refusal) return h(EmblemRefused, { reason: verdict.refusal, className: cls });
	return h(EmblemImage, { src: diceBearUrl(style, seed, options), title: verdict.credit, className: cls });
}

const LICENCE_REFUSED = "Unavailable for licensing reasons: {reason}";

function EmblemRefused({ reason, className: cls }) {
	return h(
		"span",
		{ className: cx("wg-kit-emblem-refused", cls), title: LICENCE_REFUSED.replace("{reason}", reason) },
		h(Icon, { name: "ban", size: 16 }),
	);
}

function EmblemFallback({ seed, className: cls, children }) {
	const { status } = useContext(EMBLEM_STATUS);
	if (status === "loaded" || status === "refused") return null;
	if (children) return h("span", { className: cx("wg-kit-emblem-fallback", cls) }, children);
	return h(PlaceholderMark, { seed, className: cls });
}
...
export function Spinner({ size = 18, className: cls }) {
	return h(
		"span",
		{ className: cx("wg-kit-spinner", cls), style: { width: `${size}px`, height: `${size}px` }, "aria-hidden": "true" },
		h("svg", { viewBox: "0 0 24 24" }, h("circle", { cx: 12, cy: 12, r: 9 })),
	);
}

function buttonMark(isLoading, isDone, size) {
	const px = MARK_PX[size] ?? MARK_PX.m;
	if (isDone) return h(Icon, { name: "tick", size: px, className: "wg-kit-btn-mark" });
	if (isLoading) return h(Spinner, { size: px, className: "wg-kit-btn-mark" });
	return null;
}

export function Button({ isLoading = false, isDone = false, children, ...props }) {
	...
	const held = {
		...
		children: mark === null ? children : h(Fragment, null, mark, children),
	};
...
export function Badge({ color, style, ...props }) {
...
export function Heading({ level, size, className: cls, children, ...rest }) {
...
// TODO: drop Pill once every published widget imports Badge
...
export function ShowMore({ remaining, isLoading = false, onMore, children, className: cls }) {
	...
	return h(
		Button,
		{ size: "s", block: true, className: cls, disabled: isLoading, onClick: onMore },
...
export function Layout({ kind = "stack", min = DEFAULT_CELL_PX, className: cls, style, children, ...rest }) {
	...
	if (worn === "rows" && !isOnPlate) return h(RowsWithHeadOutside, { rest, look }, held);
	if (worn === "rows") return h("div", { ...rest, ref: rowsNode, className: cx(look.className, "is-on-plate") }, held);
	return h("div", { ...rest, ...look }, held);
}

const HEAD_OUTSIDE = createContext(null);

function RowsWithHeadOutside({ rest, look, children }) {
	const [headPlace, setHeadPlace] = useState(null);
	return h(
		"div",
		{ className: "wg-kit-layout-block" },
		h("div", { ref: setHeadPlace, className: "wg-kit-layout-heads" }),
		h(HEAD_OUTSIDE.Provider, { value: headPlace }, h(Card, { ...rest, ...look }, children)),
	);
}
...
function LayoutHeader({ title, className: cls, children, ...rest }) {
	...
	const drawn = h(
		"div",
		{ ...rest, className: cx("wg-kit-layout-head", headPlace && "is-outside", cls) },
		title ? h(LayoutTitle, null, title) : null,
		title && children ? h(LayoutActions, null, children) : children,
	);
	if (headPlace) return createPortal(drawn, headPlace);
	return drawn;
}

function LayoutTitle({ level = LAYOUT_TITLE_LEVEL, size = LAYOUT_TITLE_SIZE, className: cls, ...rest }) {
	return h(Heading, { ...rest, level, size, className: cx("wg-kit-layout-title", cls) });
}

function LayoutActions({ className: cls, children, ...rest }) {
	return h("div", { ...rest, className: cx("wg-kit-layout-actions", cls) }, children);
}

const ACTION_ICON_PX = 16;

export function ActionButton({ icon, label, className: cls, children, ...rest }) {
	const isMarked = rest.isLoading || rest.isDone;
	const drawnIcon = isMarked ? null : typeof icon === "string" ? h(Icon, { name: icon, size: ACTION_ICON_PX }) : icon;
	const own = { ...rest, size: "s", className: cx("wg-kit-action", cls) };
	if (!children) return h(IconButton, { ...own, label }, drawnIcon);
	return h(Button, { ...own, "aria-label": label }, drawnIcon, children);
}
...
function useResizeWatch(ref, measure) {
	useLayoutEffect(() => {
...
function useSlider(value, onChange) {
...
function sliderPropsOf(shown, report) {
	return {
		role: "slider",
		tabIndex: 0,
		...rangeAria(shown),
...
function percentUnder(track, event) {
...
function useSliderPointer(trackRef, report) {
...
export function Progress({ value = 0, onChange, label, className: cls }) {
	...
	return h(
		"div",
		{ className: cx("wg-kit-progress", isGrabbed && "is-grabbed", cls), "aria-label": label, ...sliderProps },
		h(
			"span",
			{ className: "wg-kit-progress-track", ref: trackRef, ...trackProps },
...
const CIRCLE_AMPLITUDE = 1.6;
const CIRCLE_WAVELENGTH = 15;
const TAU = Math.PI * 2;
const PROGRESS_SHAPES = ["line", "circle"];
const STATUS_TONES = { empty: "neutral", running: "accent", done: "success" };
...
function shownValue(displayValue, shown) {
...
// TRADE-OFF: the path is drawn per pixel of width, because a stretched wave changes its wavelength
export function ProgressBar({ shape = "line", ...props }) {
	if (wornWord(shape, PROGRESS_SHAPES, "progress shape") === "circle") return h(ProgressCircle, props);
	return h(ProgressLine, props);
}

function tickWhenDone({ state }) {
	if (state !== "done") return null;
	return h(Icon, { name: "tick", size: DONE_ICON_PX });
}

export function StatusProgress({ value = 0, tones, displayValue, className: cls, ...rest }) {
	const shown = clampPercent(value);
...
function ProgressLine({
	...
	return h(
		...
		h(
			"span",
			{ className: "wg-kit-bar-line", ref: slider.trackRef },
			width > 0 &&
				h(BarDrawing, {
					width,
		...
		h("span", { className: "wg-kit-bar-value" }, h("span", { className: "wg-kit-bar-said", ref: valueRef }, said)),
	);
}

const VALUE_GAP_PX = 8;
...
function ProgressCircle({
	...
	return h(
		...
		h(
			...
			drawn.track && h("path", { className: "wg-kit-bar-track", d: drawn.track }),
			drawn.active && h("path", { className: "wg-kit-bar-active", d: drawn.active }),
		),
		h("span", { className: "wg-kit-ring-value" }, said),
	);
}

const DONE_ICON_PX = 24;
...
function BarDrawing({ width, drawn, hasStopMark }) {
	const { track, active, cursor, height, middle, stop } = drawn;
	return h(
		"svg",
		{ width, height, viewBox: `0 0 ${width} ${height}`, "aria-hidden": "true" },
		track && h("path", { className: "wg-kit-bar-track", d: track }),
		track && hasStopMark && h("circle", { className: "wg-kit-bar-stop", cx: stop, cy: middle, r: BAR_STROKE / 2 }),
		active && h("path", { className: "wg-kit-bar-active", d: active }),
		cursor &&
			h("rect", {
...
export function Switch({ checked, onChange, label, className: cls }) {
	return h("button", {
		type: "button",
		role: "switch",

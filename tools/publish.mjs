...
const { HELD_BY_THE_ENGINE, facadeUrl, realPathIn } = await import("./.mjs-cache/engine/modules.mjs");
...
export function packageNames(code, heldElsewhere) {
	...
	for (const [, specifier] of String(code).matchAll(/require\(\s*["']([^"']+)["']\s*\)/g)) {
		if (specifier.startsWith(".") || held.has(specifier)) continue;
...
export async function publishWidget({ folder, files, lockfile, askEsm }) {
	...
	let declared;
	try {
		card = readRecord(JSON.parse(files[RECORD_FILE] ?? null), id);
	...
	const found = dependenciesFrom(packageNames(code, [...HELD_BY_THE_ENGINE, `${scope}/lib`]), lockfile);
...
function filesUnder(folder) {
	const held = {};
	for (const name of [RECORD_FILE, ...SOURCE_FILES, ...SHEET_FILES]) {
...
async function runCli(folder, out) {
	...
	const done = await publishWidget({
		folder,
		files,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
	...
	await runCli(folder, named < 0 ? path.join("dist", folder) : process.argv[named + 1]);
}

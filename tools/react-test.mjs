...
function widgetFolder(vault, id, manifest) {
	vault.files.set(`${WIDGETS_DIR}/${id}/manifest.json`, JSON.stringify({ id, api: 1, title: id, props: NUMBER_PROP, ...manifest }));
...
check("neither loaded with an error", [registry.get(HOSTED)?.error ?? null, registry.get(OWN)?.error ?? null], [null, null]);
...
check("while the widget on the engine's React keeps its own count", all(".probe-press").find((node) => !node.closest(".wg-mounted")).textContent.trim(), "0");

console.log("\n— one cache, however many Reacts —");
const ownSaid = (inSeam) => all(".probe-own").find((node) => Boolean(node.closest(".wg-mounted")) === inSeam)?.textContent.trim();
...
board = normalizeBoard({ tiles: BOARD.tiles.slice(0, 1), layout: { left: [], main: [[{ id: "hosted", height: 120 }]], right: [] } });
draw();
await settle();
check("taking the tile away releases the root it drew in", [all(".wg-mounted .probe").length, globalThis.WG_GONE[REACT_18] ?? 0], [0, 1]);
check("and leaves the widget beside it mounted", [said(".probe-react"), globalThis.WG_GONE[ENGINE_REACT] ?? 0], [[ENGINE_REACT], 0]);

console.log("\n— nothing may be installed under a name the engine answers —");
let askedOfTheNetwork = 0;
const space = createModuleSpace({ adapter: fakeVault(), fetchText: async () => { askedOfTheNetwork += 1; return ""; } });
for (const name of HELD_BY_THE_ENGINE) {
	const refused = await space.take({ modules: {} }, name, "1.0.0");
	check(`"${name}" is refused before anything is fetched`, [refused.ok, refused.failure.includes(name), askedOfTheNetwork], [false, true, 0]);
}

console.log("\n— the surface takes the core, it does not carry one —");
const coreExports = await import("./.mjs-cache/api-core.mjs");
const coreHolders = [...SURFACE_SOURCE.matchAll(/var (\w+) = require\("widgetarium\/core"\)/g)].map((found) => found[1]);
const readFromCore = new Set(
	coreHolders.flatMap((holder) => [...SURFACE_SOURCE.matchAll(new RegExp(`\\b${holder}\\.(\\w+)`, "g"))].map((found) => found[1])),
);
check("the surface asks the core for what it needs", coreHolders.length > 0, true);
check("and asks for nothing the core does not export", [...readFromCore].filter((name) => !(name in coreExports)), []);
...
check("but a second instance of that very version is", reactClash(onePlace, { instance: "another", version: "19.2.8" })?.includes("19.2.8"), true);
check("a widget nobody has loaded is no clash", [reactClash(null, onePlace), reactClash(onePlace, undefined)], [null, null]);
...
const refused = resolveSlots(holder, { slots: {} }, registry, host, null).card;
check("the slot draws the refusal rather than emptiness", [typeof refused, refused?.()?.props?.children?.[1]?.props?.children], ["function", clash]);
check("while a slot filled from the same React draws", typeof resolveSlots({ id: HOSTED, slots: { card: { default: HOSTED } } }, { slots: {} }, registry, host, null).card, "function");
...
draw();
await settle();
check("both widgets draw again after the registry reloads", said(".probe-react").sort(), [REACT_18, ENGINE_REACT].sort());

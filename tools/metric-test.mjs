...
const DECLARED = propsFromTypes(fs.readFileSync(WIDGET, "utf8"), WIDGET);
...
check("peak and low read the days that carry a record, never the zero-filled ones", [steady.peak, steady.low], [20, 10]);
...
check("every record left out of the numbers is counted by why", [leftOut.undated, leftOut.unreadable, leftOut.ahead], [1, 1, 1]);
check("a record left out does not reach the total", leftOut.total, 10);

check("the same draft mints the same key", lib.keyOf({ date: TODAY, amount: 4, note: "a" }), lib.keyOf({ date: TODAY, amount: 4, note: "a" }));
check("a different note mints a different key", lib.keyOf({ date: TODAY, amount: 4, note: "a" }) === lib.keyOf({ date: TODAY, amount: 4, note: "b" }), false);

check("a thousand is written compactly", [lib.formatCompact(3154), lib.formatCompact(128000), lib.formatCompact(7)], ["3.15K", "128K", "7"]);
...
function gatewayOver(rows, verbs = ["create", "update", "remove"]) {
	...
	return mappedCollection(base, { needs: DECLARED.records.needs });
}
...
function periodsGateway() {
	...
	return collectionGateway({
		...
		settlesNow: true,
		handlers: { list: () => ({ rows: held, total: held.length }), get: (ref) => held.find((row) => row.ref === ref) ?? null },
	});
}
...
const platesShown = () => [...host.querySelectorAll(".mt-plate")].map((plate) => `${plate.querySelector(".mt-plate-value").textContent} ${plate.querySelector(".mt-plate-label").textContent}`);
const pressed = (label) => [...host.querySelectorAll("button")].find((button) => (button.getAttribute("aria-label") ?? button.textContent) === label);
...
check("the tone reaches the root, where the sheet reads it", host.querySelector(".wg-metric").getAttribute("data-tone"), "up");
...
check("a metric where rising is bad turns the other way", host.querySelector(".wg-metric").getAttribute("data-tone"), "down");
...
removable.click();
await settled();
check("pressing delete asks the gateway to remove that very row", written.map((entry) => entry.verb), ["remove"]);

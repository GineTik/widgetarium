...
// RESTS at, so the moment it was dragged it grew up through them.
{
	...
	check(
		"nothing overlaps at any height the sheet can reach",
		[CHROME.sheetPeekPx, 300, 500, 700].every((height) => {
...
console.log("— the free area is the window minus every panel, and one cell of margin —\n");
{
	...
	check(
		"and one cell below the header's band",
		free.top,
		CHROME.padPx + CHROME.headerHeightPx + CHROME.gapPx + GRID.cellPx,
	);
	check(
		"it stops one cell short of the panel",
		free.right,
		WINDOW.width - CHROME.padPx - CHROME.panelWidthPx - CHROME.gapPx - GRID.cellPx,
	);
	check(
		"and one cell above the zoom bar's band",
		free.bottom,
		WINDOW.height - CHROME.padPx - CHROME.barHeightPx - CHROME.gapPx - GRID.cellPx,
	);

	const folded = freeArea(WINDOW, { ...CHROME, panelWidthPx: CHROME.foldedPanelPx });
	check(
		"folding the panel widens the free area",
		folded.width - free.width,
		CHROME.panelWidthPx - CHROME.foldedPanelPx,
	);
}
...
console.log("\n— between the floor and 1:1 it really does zoom, and stays centred —");
{
	...
	check(
		"still centred, at the zoomed size",
		Math.round(at.x - free.left),
		Math.round((free.width - slightly.width * opening.scale) / 2),
	);
}
...
console.log("\n— the pan clamp keeps one whole cell of the widget outside both panels —");
{
	...
	check(
		"dragged left as far as it goes, a cell is still in the free area",
		Math.round(far.x + WIDE.width - free.left),
		GRID.cellPx,
	);
...
console.log("\n— on a phone the sheet takes the bottom, never the right edge —");
{
	...
	check(
		"the sheet's peek is taken off the bottom",
		sheet.bottom,
		phone.height - CHROME.padPx - CHROME.barHeightPx - CHROME.gapPx - CHROME.sheetPeekPx - CHROME.gapPx - GRID.cellPx,
	);
}
...
console.log("\n— a mounted widget can record which widget fills its slot —");
{
	...
	check(
		...
		JSON.stringify(heldTile({ id: "g" }, "mounted", "k", "w").slots),
		"{}",
	);
	// CONTEXT: the mount setting is the live truth; the record's widget is a mirror of it
	check(
		"the setting's widget wins over the record's",
		heldTile({ id: "g", mounted: { k: { widget: "@stale/one" } } }, "mounted", "k", "@live/two").widget,
		"@live/two",
	);
}
...
console.log("\n— and the panel writes what it draws —");
{
	...
	check(
		"and there are exactly three glass surfaces",
		all(".wg-set-window .wg-kit-glass").length + all(".wg-set-window .wg-kit-side.is-glass").length,
		3,
	);
	...
	check(
		"but the panel already draws the new value",
		rowSaying("Group tasks by")?.textContent.includes("assignee"),
		true,
	);
	...
	check(
		"flipping it writes the value into the draft",
		rowSaying("Compact rows")?.querySelector('[role="switch"]')?.getAttribute("aria-checked"),
		"true",
	);
	...
	await press(all(OPEN_POP + " button").find((button) => button.textContent.trim() === "Apply"));
	check(
		"the source folder is written to the draft",
		rowSaying("Tasks")?.textContent.includes("Orbitask/Archive"),
		true,
	);
	...
	check(
		...
		all(".wg-cat-dialog .wg-cat-tile").every((tile) => tile.getAttribute("aria-label").startsWith("Use ")),
		true,
	);
	...
	check(
		...
		new Set(drawn.map((tile) => tile.getAttribute("aria-label").split(" ")[0])).size,
		1,
	);
	check(
		"nothing sits under the list of candidates",
		find(".wg-cat-dialog .wg-cat-main").lastElementChild.className,
		"wg-cat-scroll",
	);
	...
	check(
		"the Data tab reports what the source can do",
	...
	const groupSaying = (label) =>
		all(".wg-set-group").find((node) => node.querySelector(".wg-kit-side-label")?.textContent === label);
	...
	await press(tab("Data"));
	check(
		"so the actions stay on, because the default is the folder now",
		canRow("Create")?.textContent.includes("Orbitask/Tasks"),
		true,
	);

	await press(tab("Design"));
	check("the Design tab counts no cells, because a tree cell has none to count", Boolean(rowSaying("Width")), false);
	...
	check(
		...
		`${Math.round(screenFit.scale * 100)}%`,
	);
	...
	await press(bar("1:1"));
	check(
		"1:1 removes the transform the popovers and the glass cannot survive",
		find(".wg-set-body")?.style.transform,
		"none",
	);
	...
	check(
		"and it carries none of the window\u2019s geometry",
		mount.querySelector(".wg-tile-body")?.getAttribute("style") || "",
		"",
	);
	...
	// re-flowed stayed re-flowed — the board came back a different shape from the one it went in.
	{
		...
		await press(tab("Settings"));
		await press(rowSaying("Tasks"));
		const elsewhere = find(OPEN_POP + " input");
		elsewhere.value = "Orbitask/Elsewhere";
...
console.log("\n— an unfed child is a level of its own, and the trail is the way back —");
{
		...
		tiles: [
			{
				id: "t1",
				widget: GROUP_ID,
				settings: { views: `${KANBAN_ID}, ${ARCHIVE_ID}` },
				mounted: { [ARCHIVE_ID]: { settings: { since: "2020" } } },
			},
			{ id: "t2", widget: GROUP_ID, settings: { views: KANBAN_ID } },
		],
		layouts: {
			20: [
				{ id: "t1", x: 0, y: 0, w: 9, h: 6 },
				{ id: "t2", x: 9, y: 0, w: 9, h: 6 },
			],
		},
	});
	const mount = document.getElementById("host");
	...
	check(
		"the sibling view carries one too",
		Boolean(rowSaying("Archived columns")?.querySelector(".wg-set-enter")),
		true,
	);
	...
	check(
		...
		board.tiles[0].mounted[KANBAN_VIEW].props.groupBy.value,
		"assignee",
	);
	check(
		"and the grandchild's into the grandchild's",
		board.tiles[0].mounted[KANBAN_VIEW].slots.panel.props.width.value,
		"wide",
	);
	...
	await press(settingsButtons()[0]);
	check(
		"the mount group is labelled by the manifest, not by its setting key",
		Boolean(all(".wg-set-panel .wg-kit-side-label").find((node) => node.textContent.trim() === "Views")),
		true,
	);
	...
	check(
		"and the row it collided with keeps its own name",
		all(".wg-set-panel .wg-kit-row").filter(
			(row) => row.textContent.includes("Archived columns") && !row.textContent.includes("Archived columns 2"),
		).length,
		1,
	);
	...
	check("Done writes the new shape, a row per name", holds, [
		{ name: "Archived columns 2", widget: KANBAN_ID },
		{ name: "Archived columns", widget: ARCHIVE_ID },
	]);
	...
	check(
		...
		board.tiles[0].mounted["Archived columns 2"]?.props?.groupBy?.value,
		"assignee",
	);
...
console.log("\n— a folder's readers are counted by the widget in the record, not by its key —");
{
	...
	let board = normalizeBoard({
		...
		layouts: {
			20: [
				{ id: "alone", x: 0, y: 0, w: 9, h: 6 },
				{ id: "group", x: 9, y: 0, w: 9, h: 6 },
			],
		},
	});
	const mount = document.getElementById("host");
	...
	check(
		"a widget mounted under a NAME still counts as a reader of its folder",
		notes.includes("This folder is read by 2 widgets on this board."),
		true,
	);
...
console.log("\n— a prop renamed in the manifest still finds the folder the tile chose —");
{
	...
	draw();
	check(
		"the gateway reads the folder stored under the old key",
		[asked.includes(CHOSEN), asked.includes(DECLARED)],
		[true, false],
	);
	...
	await press(document.querySelector(".wg-set-panel .wg-set-row"));
	check(
		"the window offers the folder that was chosen, not the declared one",
		document.querySelector(".wg-set-panel .wg-set-row").textContent.includes(CHOSEN),
		true,
	);

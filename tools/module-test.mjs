...
check(
	"react is asked to stay outside the bundle",
	facadeUrl(PACKAGE, "^6.3.1"),
	`https://esm.sh/${PACKAGE}@%5E6.3.1?bundle&external=react,react-dom`,
);
...
check("a manifest declares its packages as a map", declaredDependencies({ dependencies: { [PACKAGE]: "^6.3.1" } }), [
	[PACKAGE, "^6.3.1"],
]);
check(
	"and anything else declares none",
	[declaredDependencies({}), declaredDependencies({ dependencies: ["x"] }), declaredDependencies(null)],
	[[], [], []],
);
...
check(
	"a widget is told which name resolves to which version",
	[...(modulesByWidget(pointed).get("@task/board") ?? [])],
	[[PACKAGE, KEY]],
);
...
check(
	"one folder holds it",
	[...vault.files.keys()].filter((path) => path.startsWith(`${MODULES_DIR}/`)),
	[modulePath(KEY)],
);
check("and both widgets point at that one entry", (await installer.lock()).modules[KEY].widgets, [
	"@demo/clock",
	"@task/board",
]);
...
await registry.load();
check(
	"a widget importing a locked package loads",
	[Boolean(registry.get("@demo/clock")?.component), String(registry.get("@demo/clock")?.error ?? "")],
	[true, ""],
);
check(
	"and what it draws came out of the package",
	registry.get("@demo/clock").component({}).props.children,
	"dragging",
);
check(
	"the second one draws from the same version",
	registry.get("@task/board").component({}).props.children,
	"dragging",
);
...
check(
	"and names the package in the refusal",
	String(alone.get("@demo/clock")?.error ?? "").includes(`cannot import "${PACKAGE}"`),
	true,
);

const moved = fakeVault();
for (const [path, text] of vault.files)
	moved.files.set(path === modulePath(KEY) ? `${MODULES_DIR}/elsewhere.js` : path, text);
...
unlocked.files.set(
	LOCK_PATH,
	JSON.stringify({ version: 1, widgets: JSON.parse(unlocked.files.get(LOCK_PATH)).widgets, modules: {} }),
);
const without = new WidgetRegistry({ vault: { adapter: unlocked } });
await withoutTheReport(() => without.load());
check(
	"a widget whose lock entry is gone refuses rather than drawing",
	Boolean(without.get("@demo/clock")?.error),
	true,
);
check(
	"and names the package it was promised",
	String(without.get("@demo/clock")?.error ?? "").includes(`cannot import "${PACKAGE}"`),
	true,
);
...
check(
	"uninstalling the last one takes the folder with it",
	[...vault.files.keys()].filter((path) => path.startsWith(`${MODULES_DIR}/`)),
	[],
);
...
check(
	"an unscoped package stands beside a scoped one",
	[...mixed.files.keys()].filter((path) => path.startsWith(`${MODULES_DIR}/`)).sort(),
	[modulePath("clsx@2.1.1"), modulePath(KEY)].sort(),
);

const lying = fakeVault();
const liar = createInstaller({
	adapter: lying,
	...network({ ...SERVED, [facadeUrl(PACKAGE, "^6.3.1")]: `export * from "/@evil/miner@1.0.0/es2022/miner.mjs";` }),
});
...
check(
	"and says which package was asked for",
	refusedPackage.failure,
	`esm.sh answered "${PACKAGE}@^6.3.1" with something that is not that package`,
);
check(
	"and nothing of the widget reached the vault",
	[...lying.files.keys()].filter((path) => path.startsWith(".widgetarium/widgets")),
	[],
);
...
check(
	"a bundle that will not come back is a refusal, not a crash",
	[lost.ok, lost.failure.startsWith(`cannot fetch "${PACKAGE}@^6.3.1"`)],
	[false, true],
);

const escaping = fakeVault();
const escaped = await createInstaller({
	adapter: escaping,
	...network({
		...SERVED,
		"https://raw.githubusercontent.com/acme/widgets/abc1234567/widgets/@demo/clock/manifest.json": JSON.stringify({
...
check(
	"and nothing was asked of the network for it",
	asked.includes("https://esm.sh/../../evil@1.0.0?bundle&external=react,react-dom"),
	false,
);

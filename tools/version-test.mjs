...
const { BLOCK_FORMAT, WIDGET_API, MIN_WIDGET_API, blockFormatOf, blockRefusal, widgetApiOf, apiRefusal } = await import("./.mjs-cache/version.mjs");
...
check("a v that is not a number is refused", blockRefusal({ v: "two", tiles: [] }), 'Widgetarium: this board declares format "two", which is not a version number.');
check("a v below the first format is refused", blockRefusal({ v: 0, tiles: [] }), "Widgetarium: this board declares format 0, and board formats start at 1.");
check("a negative v is refused", blockRefusal({ v: -1, tiles: [] }), 'Widgetarium: this board declares format "-1", which is not a version number.');
check("a fractional v is refused", blockRefusal({ v: 1.5, tiles: [] }), 'Widgetarium: this board declares format "1.5", which is not a version number.');

const board = normalizeBoard({ tiles: [{ id: "a", widget: "@demo/clock" }], layouts: { 12: [{ id: "a", x: 0, y: 0, w: 3, h: 2 }] } });
...
check("a board round-trips through its own stamp", normalizeBoard(serializeBoard(board)).tiles.map((tile) => tile.id), ["a"]);
...
check("an api that is not a number is refused", apiRefusal({ id: "@demo/clock", api: "1.0" }), '@demo/clock declares widget API "1.0", which is not a version number.');
check("a negative api is refused", apiRefusal({ id: "@demo/clock", api: -1 }), '@demo/clock declares widget API "-1", which is not a version number.');
check("a fractional api is refused", apiRefusal({ id: "@demo/clock", api: 1.5 }), '@demo/clock declares widget API "1.5", which is not a version number.');

const shipped = [];
for (const scope of fs.readdirSync("widgets", { withFileTypes: true }).filter((entry) => entry.isDirectory())) {
	for (const folder of fs.readdirSync(path.join("widgets", scope.name), { withFileTypes: true }).filter((entry) => entry.isDirectory())) {
		const at = path.join("widgets", scope.name, folder.name, "manifest.json");
...
check("every shipped widget declares an api", shipped.filter((entry) => entry.manifest.api === undefined).map((entry) => entry.at), []);
check("and every shipped widget runs on this plugin", shipped.filter((entry) => apiRefusal(entry.manifest)).map((entry) => entry.at), []);
...
{
	const registry = new WidgetRegistry(vaultOf(filesFor({ id: "@demo/probe", title: "Probe", api: WIDGET_API + 1, was: "@demo/older" })));
	await registry.load();
	const entry = registry.get("@demo/probe");
	check("a widget outside the range does not mount", entry?.component ?? null, null);
	check("the tile is handed the refusal, named, with both numbers", String(entry?.error?.message ?? ""), `@demo/probe needs widget API ${WIDGET_API + 1}, and this Widgetarium provides ${WIDGET_API}.`);
	check("its manifest survives, so the tile can still name it", entry?.manifest?.title, "Probe");
	check("and its old id still resolves to the refusal, not to nothing", registry.get("@demo/older")?.manifest?.id, "@demo/probe");
}
...
function fakeVault() {
	...
	return {
		...
		write: async (at, text) => { files.set(at, text); },
		mkdir: async () => {},
	};
}
...
const offer = { manifest: { id: "@demo/clock", repository: "https://github.com/acme/widgets", ref: "main", path: "widgets/@demo/clock", files: ["manifest.json", "widget.jsx"] } };

{
	...
	check("and the refusal names the widget and both numbers", refused.failure, `@demo/clock needs widget API ${WIDGET_API + 1}, and this Widgetarium provides ${WIDGET_API}.`);
...

{
	...
	const disk = {
		...
		read: async (at) => (at.endsWith("manifest.json") ? JSON.stringify({ id: "@demo/clock", title: "Clock", api: WIDGET_API + 1 }) : WIDGET),
	...
	check("and says so with the widget and both numbers", refused.failure, `@demo/clock needs widget API ${WIDGET_API + 1}, and this Widgetarium provides ${WIDGET_API}.`);
	check("and nothing of it reached the vault", vault.files.size, 0);
}

const { default: WidgetariumPlugin, drawable } = await import("./.mjs-cache/main.mjs");

const offered = (api) => drawable({ manifest: { id: "@demo/clock", api }, code: WIDGET, path: "@demo/clock/widget.jsx" });
...
check("and the card is handed the refusal, named, with both numbers", String(offered(WIDGET_API + 1).error?.message ?? ""), `@demo/clock needs widget API ${WIDGET_API + 1}, and this Widgetarium provides ${WIDGET_API}.`);
...
check("and the note says why, with both numbers", newer.said, [`Widgetarium: this board was written in format 99, and this plugin reads up to ${BLOCK_FORMAT}. Update Widgetarium to open it — nothing was changed.`]);

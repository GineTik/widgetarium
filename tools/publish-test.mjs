...
check(
	...
	(await publishWidget({ folder: FOLDER, files: cjsOnly, lockfile: LOCKFILE, askEsm: servingEsm })).record.dependencies,
	{ "tidy-cjs": "^2.0.0" },
);
...
const installedAnyway = await unpublishedInstaller.install(
	(await unpublishedInstaller.discover({ path: "/repo/widgets" }))[0],
);
check(

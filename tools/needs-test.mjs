...
const HABITS = [
	{ path: "Habits/Reading.md", name: "Reading", props: { entries: ["2026-08-01", "2026-08-02"], name: "Reading", target: 21 } },
...

{
	...
	check("fields: a list of days reports its element type, not just list", entries?.type === "list" && entries?.elementType === "date" && entries?.many === true, JSON.stringify(entries));
...

{
	...
	check("ladder: the only field of the wanted type is taken even under another name", map.goal === "target", JSON.stringify(map));
	check("ladder: nothing is missing while every required need resolved", missing.length === 0 && unresolved.length === 0);
}

{
	...
	check("drift: an unrecognisable name still resolves while it is the only date list", map.days === "kept", JSON.stringify(map));
}

{
	const twoLists = HABITS.map(({ props, ...rest }) => ({ ...rest, props: { name: props.name, kept: props.entries, skipped: ["2026-08-09"] } }));
	const { map, missing } = resolveNeeds(NEEDS, fieldsOf(twoLists));
	check("drift: a second list of days is the question, not a guess", map.days === undefined && missing.includes("days"), JSON.stringify(map));
}
...
{
	const { map } = resolveNeeds({ due: { type: "date", aka: ["due date"] } }, fieldsOf([{ props: { "due-date": "2026-08-01" } }]));
...

{
	...
	check("mapped: the widget reads the need, never the property", Array.isArray(first.days) && first.days.length === 2, JSON.stringify(first.days));
	check("mapped: the record keeps what it already carried", first.path === "Habits/Reading.md" && first.name === "Reading");
...

{
	...
	const listed = await mapped.list();
	check("coercion: one day where many were declared arrives wrapped", JSON.stringify(listed.rows[0].value.days) === '["2026-08-01"]');
}

{
	const { gateway: base } = folderStandIn([{ path: "a.md", props: { entries: ["2026-08-01", "not a day", ""], name: "One" } }]);
	const mapped = gateway.mappedCollection(base, { needs: NEEDS });
	const listed = await mapped.list();
	check("coercion: a value that will not convert is dropped, not drawn", JSON.stringify(listed.rows[0].value.days) === '["2026-08-01"]');
}

{
	...
	check("mapped: a write goes back under the property the vault uses", written[0]?.data?.props?.entries?.[0] === "2026-08-04", JSON.stringify(written[0]));
...

{
	...
	check("ladder: a required need nothing answers is reported missing", seen.missing.includes("colour"), JSON.stringify(seen));
}

{
	...
	check("write: a need no property answers is refused, never dropped in silence", refusal !== null, JSON.stringify(written));
	check("write: the refusal names the need the person would look for", String(refusal?.message ?? "").includes("colour"), String(refusal?.message));
...

{
	...
	check("read: a condition on an unanswered need narrows nothing rather than to nothing", listed.rows.length === HABITS.length, String(listed.rows.length));
}

{
	...
	check("read: a condition on an answered need is asked under the vault's own property", listed.rows.length === 1, String(listed.rows.length));
}

...
const { BUILD_FILE } = await import("./.mjs-cache/engine/widget-build.mjs");
...
const build = vault.files.get(`${FOLDER}/${BUILD_FILE}`);
check("and leaves a build beside the source", typeof build, "string");
check("which holds no JSX", String(build).includes("<b>"), false);
check("and no type annotation", String(build).includes(": string"), false);
check("the lock names the source that build was made from", (await installer.lock()).widgets["@demo/clock"].build, { from: SOURCE_FILE });
...
vault.files.set(`${FOLDER}/${BUILD_FILE}`, buildSaying("from the build"));

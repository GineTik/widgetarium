	...
	return {
		...
		setup(build) {
			...
			build.onLoad({ filter: /.*/, namespace: "wg-types" }, () => ({
				...
				loader: "js",
				watchFiles: ["widgets/types/widgetarium.d.ts", "widgets/types/globals.d.ts", "tsconfig.widgets.json"],
			}));
		},
	};
}
...
const HELD_BY_THE_ONE_CORE = /^\.\/(cache|create|narrow|emoji-table\.js|icon-table\.js)$/;

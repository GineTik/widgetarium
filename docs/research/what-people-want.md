...
`@inline/rating`, `@inline/status`, `@inline/slider`, `@inline/date` — but the point is not the list,
...
earns its place as a *demonstration that the mechanism is general*, not as a growth bet — and it
...
**Cost.** Medium, and mostly conceptual. The write-back exists; the *semantics* do not match. Our
...
1. **The trigger cannot be `![[…]]`** — see §4.1. By post-processor time the embed is a DOM node,
   not text.
2. **Editing a *different* note** needs a writer for a file the user never bound. `passageHere` owns
...
**Recommendation: do not chase it.** Cite it as proof the *category* is alive, and let it justify
...
`![[main.py]]`. But a post-processor sees the DOM *after* markdown parsing, and Obsidian turns an
...
both of which the existing `line` and `wrapped` modes already do. Delivering the *feature* costs
nothing extra; delivering the *syntax* means intercepting embeds, which is a different plugin.
...
specific risk remains open — Prism language components can load lazily, so a language's *first*

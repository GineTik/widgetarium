...
## A widget's own version is a commit

A widget manifest carries no version. A tile installed from a repository names the commit it was made
with, `widget: "@scope/name@<commit>"`, and a widget living in the vault is named by its bare id. An
update is never trusted to say what it changed: the engine compares the props of the installed card
with the served one (`src/engine/compatibility.js`).

| Change                                                             | Tiles                                                                    |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| label, hint, description, code                                     | follow it in place                                                       |
| a prop added with a default, a verb added, a rename carrying `was` | follow it in place                                                       |
| a changed default                                                  | move to it by a press, with no migration                                 |
| a removed prop, a rename without `was`, a changed kind or type     | move to it only through a `migration` whose `from` names their old props |

A compatible update replaces the files and its commit joins the generation's `commits` in the lock,
so tiles naming either commit resolve to the same folder. An incompatible one installs beside it as
`@scope/name@<commit>`, and tiles stay on the generation they were made with. Until the first release
every number above — `BLOCK_FORMAT`, `WIDGET_API`, `REGISTRY_FORMAT` — is reset to 1 on release day.

...
outer region selects *what kind of thing* the inner one lists, that is a named pattern:
...
The root row's `foldable` side box is the sidebar; the `keep` middle is the content. Nothing in the

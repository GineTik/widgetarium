...
The test is one question: **does the user need it *while* acting on the main content?** If yes, it
...
Below a drawer there is only a pushed screen. For supporting content Material prefers going *down*
rather than *over*: below the focus pane, or a bottom sheet.
...
Already built, and already a law of the board: below `MAIN_FLOOR_PX` a `foldable` region stops being

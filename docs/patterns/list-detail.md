...
scanned, a list–detail is *selected from*.
...
- **The relationship is dependence, not parenthood.** If the second region is only meaningful
  *about* the first rather than *inside* it, you want a [supporting-pane](supporting-pane.md).
...
The root row with a `keep` middle and one `foldable` side is already this shape. The side box is the

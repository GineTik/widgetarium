...
Widgetarium has no overlay for this. What it has instead is better for most cases: a `foldable`
region that is *already* the detail, fed by a ref from the list — a permanent panel rather than a
summoned one.

import { canDo, ConfirmDialog, createWidget, Dialog, DialogClose, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, useData, WidgetRoot } from "widgetarium";
import type { Action, Aka, CollectionGateway, CreateAction, GetAction, ListAction, RemoveAction, Row, Text, UpdateAction, ValueGateway, VaultRecord, ViewHost } from "widgetarium";
...
import { cardSizeOf, DEFAULT_TIERS, freeLabel, isLabelTaken, labelOf, nameOf, nextToneAfter, orderBetween, pictureOf, placedAt, PRESETS, rackOf, renumbered, rowsOf, toneForSeed, toneOf } from "@rank/lib";
import type { Picture, Preset } from "@rank/lib";
...
type CardWrites = {
	...
	update?: UpdateAction;
	remove?: RemoveAction;
	replace?: Action<Row<Partial<CardRecord>>[], void>;
};
...
type TierWrites = {
	...
	update?: UpdateAction;
	remove?: RemoveAction;
	replace?: Action<Row<Partial<TierRecord>>[], void>;
};

type TierListProps = {
	cards: CollectionGateway<CardRecord, CardWrites>;
	tiers: CollectionGateway<TierRecord, TierWrites>;
	title: ValueGateway<string, { get: GetAction; update?: UpdateAction }>;
	cardSize: ValueGateway<number, { get: GetAction; update?: UpdateAction }>;
	host?: ViewHost;
};
...
type Gates = { cards: CollectionGateway<CardRecord, CardWrites>; tiers: CollectionGateway<TierRecord, TierWrites>; say: (said: string) => void };
...
const RENAME_REWRITES = "The name is what the cards point at, so renaming this row also rewrites the {count} cards in it.";
...
const RESET_SAID = "Every card leaves the row it is in and returns to the tray. The rows, their names and their colours stay exactly as they are, and no card is deleted.";
const RESET_CONFIRM = "Put them back";
const REPLACE_TITLE = "Replace what is here with {name}?";
const REPLACE_SAID = "The {cards} cards and {rows} rows in this tile are dropped, and the preset's own take their place.";
...
const CARD_PICTURE_SAID = "A web address, an attachment in this vault, or emoji: followed by a face name. Left empty, the card draws its letters.";
const CARDS_COUNTED = "{count} cards";

const Renderer = createContext<RenderMarkdown | null>(null);

export default createWidget(function TierList({ cards, tiers, title, cardSize, host }: TierListProps) {
	const listedCards = useData(cards.list);
	const listedTiers = useData(tiers.list);
	const heading = String(useData(title.get).data ?? "");
	const size = cardSizeOf(useData(cardSize.get).data);

	const cardRows = listedCards.rows as CardRow[];
	const tierRows = listedTiers.rows as TierRow[];
	...
	return (
		<WidgetRoot className="wg-rank" ref={rootRef} style={{ "--wg-rank-card": `${size}px` } as Record<string, string>}>
		...
		tiers: {
			...
			item: {
				fields: [
					...
					{ key: "tone", label: "Colour", type: "text" },
					{ key: "order", label: "Order", type: "number" },
				],
			},
			default: { value: DEFAULT_TIERS },
		},
		title: {
			type: "text",
			label: "Title",
			default: { value: "Tier list" },
		},
		cardSize: {
			type: "number",
			design: true,
...
function mayDo(cards: CollectionGateway<CardRecord, CardWrites>, tiers: CollectionGateway<TierRecord, TierWrites>): May {
	return { preset: canDo(cards.replace) && canDo(tiers.replace), edit: canDo(cards.update), add: canDo(cards.create), addRow: canDo(tiers.create) };
}
...
function useCardWriting(held: RackView, { cards, say }: Gates) {
	const listOf = (tier: string | null) => (tier === null ? held.tray : (held.rack.find((line) => line.label === tier)?.cards ?? []));

	const renumberCards = async (rows: CardRow[], tier: string | null) => {
		for (const row of renumbered(rows)) await cards.update({ ref: row.ref, data: { tier, order: row.value.order } });
	};
...
function useRowWriting(held: RackView, { cards, tiers, say }: Gates, listOf: (tier: string | null) => CardRow[]) {
	...
	return {
		addRow: async () => {
			...
			await tiers.create({ label: freeLabel(standing), tone: nextToneAfter(toneOf(last?.value)), order: held.tiers.length + 1 });
		},

		row: async (row: TierRow, label: string, tone: string) => {
			const was = labelOf(row.value);
			const wanted = label.trim();
			if (!wanted) return;
			if (!canDo(tiers.update)) return say(CANNOT_WRITE_ROW);
			const isRenamed = wanted !== was;
			const refusal = refusalForRename({ standing, isRenamed, wanted, heldCards: listOf(was).length, canWriteCards: canDo(cards.update) });
		...
		moveRow: async (row: TierRow, step: number) => {
			...
			for (const renumberedRow of renumbered(line)) await tiers.update({ ref: renumberedRow.ref, data: { order: renumberedRow.value.order } });
		},
...
function useDragging(rootRef: { current: HTMLDivElement | null }, onDrop: (row: CardRow, target: Target) => void) {
	...
	const grab = (row: CardRow) => (event: ReactPointerEvent<HTMLElement>) => {
		...
		let held: Carry = { row, x: start.x, y: start.y, offX: start.x - box.left, offY: start.y - box.top, isDragging: false };
		let aimed: Target | null = null;

		const follow = (moved: PointerEvent) => {
			const isPastSlop = Math.abs(moved.clientX - start.x) > TAP_SLOP_PX || Math.abs(moved.clientY - start.y) > TAP_SLOP_PX;
...
type BoardProps = { heading: string; held: RackView; opened: Opened; write: Writing; may: May; rootRef: { current: HTMLDivElement | null }; size: number; cards: CardRow[] };
...
function Board({ heading, held, opened, write, may, rootRef, size, cards }: BoardProps) {
	...
	const picking = { drag, picked: opened.picked, onPressCard: opened.pick, onEditCard: may.edit ? opened.editCard : null, onPlace: place };

	return (
		<>
			<Head heading={heading} cards={cards.length} ranked={held.ranked} may={may} opened={opened} />
			<Rack held={held} {...picking} onNameRow={opened.nameRow} onMoveRow={write.moveRow} onRemoveRow={write.removeRow} onAddRow={may.addRow ? write.addRow : null} />
			<Tray held={held} {...picking} onAddCard={may.add ? opened.addCard : null} onPresets={may.preset ? opened.pickPreset : null} onUnorphan={may.edit ? write.unorphan : null} />
			{drag.carry?.isDragging ? <Carried carry={drag.carry} size={size} /> : null}
		</>
	);
}

function Head({ heading, cards, ranked, may, opened }: { heading: string; cards: number; ranked: number; may: May; opened: Opened }) {
	return (
...
function Rack({ held, onNameRow, onMoveRow, onRemoveRow, onAddRow, ...picking }: Picking & { held: RackView; onNameRow: (row: TierRow) => void; onMoveRow: (row: TierRow, step: number) => void; onRemoveRow: (row: TierRow) => void; onAddRow: (() => void) | null }) {
				...
				{isEmpty ? (
					...
					held.rack.map((line, at) => (
						<div className={`wr-tier ${toneClass(line.tone)}`} key={line.row.ref}>
							<div className="wr-rail" onClick={() => (picking.picked ? picking.onPlace(line.label) : onNameRow(line.row))}>
								<span className="wr-rail-label">{line.label}</span>
								<span className="wr-grips">
									<Grip variant="is-up" label="Move this row up" isOff={at === 0} onPress={() => onMoveRow(line.row, -1)} />
									<Grip variant="is-down" label="Move this row down" isTurned isOff={at === held.rack.length - 1} onPress={() => onMoveRow(line.row, 1)} />
...
function Grip({ variant, label, icon = "arrow-up", isOff = false, isTurned = false, onPress }: { variant: string; label: string; icon?: string; isOff?: boolean; isTurned?: boolean; onPress: () => void }) {
	return (
		<button
			type="button"
...
function Pen({ name, cards, drag, picked, onPressCard, onEditCard, onPlace, after }: Picking & { name: string | null; cards: CardRow[]; after?: ReactNode }) {
	...
	return (
		<div className={`wr-pen${slotAt === -1 ? "" : " is-target"}`} data-pen={name ?? ""} onClick={() => (picked ? onPlace(name) : undefined)}>
			{shown.map((row, at) => (
...
function Card({ row, drag, isPicked, onPress, onEdit }: { row: CardRow; drag: Dragging; isPicked: boolean; onPress: (ref: string) => void; onEdit: ((row: CardRow) => void) | null }) {
	return (
		<div
		...
		>
			<Face card={row.value} />
			<span className="wr-cap">{nameOf(row.value)}</span>
			{onEdit ? (
				<button
					type="button"
					className="wr-edit"
...
function Face({ card }: { card: CardRecord }) {
	...
	if (drawn.kind === "remote" && !hasFailed) return face(<img src={drawn.src} alt="" draggable={false} onError={() => setFailed(true)} />);
...
function Carried({ carry, size }: { carry: Carry; size: number }) {
	const tone = toneForSeed(nameOf(carry.row.value));
	return createPortal(
		<div className={`wg-portal wg-rank wr-carried ${toneClass(tone)}`} style={{ left: `${carry.x - carry.offX}px`, top: `${carry.y - carry.offY}px`, "--wg-rank-card": `${size}px` } as Record<string, string>}>
			<Face card={carry.row.value} />
		</div>,
		document.body,
	);
}

function Orphans({ rows, drag, picked, onPressCard, onEditCard, onUnorphan }: Omit<Picking, "onPlace"> & { rows: CardRow[]; onUnorphan: (() => void) | null }) {
	return (
		<div className="wr-orphans">
			...
			<div className="wr-tray-row">
				{rows.map((row) => (
					<Card key={row.ref} row={row} drag={drag} isPicked={picked === row.ref} onPress={onPressCard} onEdit={onEditCard} />
				))}
			</div>
		</div>
	);
...
function Tray({ held, onAddCard, onPresets, onUnorphan, ...picking }: Picking & { held: RackView; onAddCard: (() => void) | null; onPresets: (() => void) | null; onUnorphan: (() => void) | null }) {
	...
	return (
		<div className="wr-tray">
			{held.orphans.length > 0 ? <Orphans rows={held.orphans} onUnorphan={onUnorphan} drag={picking.drag} picked={picking.picked} onPressCard={picking.onPressCard} onEditCard={picking.onEditCard} /> : null}
...
function TrayEmpty({ hasNothing, onAddCard, onPresets }: { hasNothing: boolean; onAddCard: (() => void) | null; onPresets: (() => void) | null }) {
	return (
...
function Windows({ held, opened, write, may, cards, rows }: { held: RackView; opened: Opened; write: Writing; may: May; cards: number; rows: number }) {
	const namedCards = opened.naming ? (held.rack.find((line) => line.row.ref === opened.naming?.ref)?.cards.length ?? 0) : 0;
...
function Asks({ opened, write, ranked, cards, rows }: { opened: Opened; write: Writing; ranked: number; cards: number; rows: number }) {
	return (
		<>
			<ConfirmDialog
...
function PresetDialog({ isOpen, onClose, onPick }: { isOpen: boolean; onClose: () => void; onPick: (preset: Preset) => void }) {
	return (
...
function CardDialog({ isOpen, row, onClose, onSave, onRemove }: { isOpen: boolean; row: CardRow | null; onClose: () => void; onSave: (draft: Draft) => void; onRemove: (() => void) | null }) {
	const [draft, setDraft] = useState<Draft>({ name: "", picture: "" });

	useEffect(() => {
		if (!isOpen) return;
		setDraft({ name: row ? nameOf(row.value) : "", picture: row ? String(row.value.picture ?? "") : "" });
	}, [isOpen, row?.ref]);
		...
		<Dialog isOpen={isOpen} onOpenChange={onClose}>
			<DialogContent className="wg-rank" width="26rem">
				...
				<div className="wr-pop">
					...
					<Field value={draft.picture} placeholder="Picture" onInput={(next: string) => setDraft({ ...draft, picture: next })} />
				</div>
				<DialogFooter>
					{onRemove ? (
...
function RowDialog({ row, heldCards, onClose, onSave }: { row: TierRow | null; heldCards: number; onClose: () => void; onSave: (label: string, tone: string) => void }) {
	const [label, setLabel] = useState("");
	const [tone, setTone] = useState("neutral");

	useEffect(() => {
		if (!row) return;
		setLabel(labelOf(row.value));
		setTone(toneOf(row.value));
	}, [row?.ref]);
				...
				<div className="wr-pop">
					...
					<div className="wr-swatches">
						{TONE_NAMES.map((name: string) => (
							<button type="button" key={name} aria-label={name} className={`wr-swatch ${toneClass(name)}${name === tone ? " is-on" : ""}`} onClick={() => setTone(name)} />
						))}
					</div>
				</div>
				<DialogFooter>

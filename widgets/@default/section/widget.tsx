...
.wg-section-empty { font-size: var(--font-ui-small, 13px); color: var(--text-muted); }
`;
...
export const manifest = defineManifest({
	...
	role: "layout",
	size: { collapseBelowPx: 200, stackBelowPx: 360 },
	mounts: {
		controls: {
			label: "Controls",
...
function Stands({ entry }: { entry: MountEntry }) {
	const style = entry.height ? { height: `${entry.height}px` } : undefined;
	if (entry.surface)
		return (
			<Card type={entry.surface} className="wg-section-stands" style={style}>
				<Mounted entry={entry} />
			</Card>
		);
	return (
		<Layout.Item className="wg-section-stands" style={style}>
			<Mounted entry={entry} />
		</Layout.Item>
	);
}

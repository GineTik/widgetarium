...
export const manifest = defineManifest({
	...
	role: "collection",
	size: { collapseBelowPx: 220, stackBelowPx: 280 },
	preview: {
		size: { w: 6, h: 4 },
		props: {
			records: {
...
export default createWidget(manifest, ({ records, columns, rowTitle, yesNo, rowsAtFirst, fadeEdges }) => {
	...
	if (read.data.length === 0) return <Line tone="var(--text-muted)" text={NO_RECORDS} />;

	const shownColumns = shownColumnsOf(declared, read.data, titleProperty);
	if (shownColumns.length === 0 && titleProperty === "") return <Line tone="var(--text-muted)" text={NO_COLUMNS} />;

...
tile.props.tasks = {
  ...
  field: "content" | "name" | "<property>",   a value prop over one note: which part of it is read
...
value      + vault     → fileGateway(path, field)   one note, or one part of it
...
```json
"selection": { "kind": "value", "of": "boards", "wants": "@default/editable-tabs/selection" }
...
- `@default/editable-tabs` declares `tabs`, `label`, `value` and `selection`.
- `@default/view-tabs` declares `options` and `selection`; both are normally bound by ref to a view
  group's `holds` and `selection`, so the group's mount list is the only list of views there is.
- `@default/view-group` exposes `holds` as a read-only collection and declares `selection` over it.
- `@default/filter-panel` declares `chosen`, a memory box; a widget narrows itself by pointing a
  `spread` row at it.
- `@default/kanban-board` declares `selection` (which board) and `opened` (which card). It draws the

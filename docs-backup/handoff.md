...
**The two inline confirms at `widgets/@default/kanban-board/widget.jsx` ~340 and ~388 are not
...
- **setting** — `@default/editable-tabs` over its own `tabs` / `archived` settings
...
- **holds** — `@default/view-group` over its mount rows, through `configureMounts` (`src/surface.js`),
...
- **Do not read `src/surface.js`, `src/model.js`, `src/kit.js`, `styles.css`, `src/settings-window.js`
  or `widgets/@default/kanban-board/widget.jsx` whole.** `grep -n` to locate, then narrow ranged reads.

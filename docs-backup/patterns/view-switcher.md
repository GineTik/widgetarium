...
This is a swap box — `dir: swap` in the layout, each child a named view, the box's own strip or a
`@default/view-tabs` switcher bound to `<id>/holds` and `<id>/selection`. Every view is an ordinary tile,
so it owns its own props; the views off screen stay mounted. See [the board](../ai/board.md).

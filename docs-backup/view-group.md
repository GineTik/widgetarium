...
Superseded: views are a swap box in the layout now (`docs/board-tree.md`, "Swap: views as a box"). What follows is the group's record, kept for the migration that reads it.
...
- **Fed** — the parent resolves the data and hands it down, so the child owns nothing and there is
  nothing inside it to configure. `@default/kanban-board` gives its `card` slot a task; that is what
...
id derived from a setting value (`@default/kanban-board`, `#2` on a repeat). Merged today, the map
...
`slots: { card: "@default/task-card" }`. Every note in the vault is written that way, and every one of
...
the host hands down beside `catalogue`. A widget cannot do this itself: it owns no tiles. The board
